...
The results of those algorithms where the set of preferences was restricted in size ($PB1c$ and $PB1d$) compare relatively bad to the base algorithm and this is most pronounced in the case of $PB1c(5\%)$ where the restriction is the strongest. This means that, at least for this benchmark, restricting the number of preferences completely backfired.
...
This result is difficult to explain. The smaller set of preferences could be less flexible than a larger one and thus more difficult to get to work. When a model is generated, not all of the preferences will be taken into account. Many of the variables that have a preferred assignment might be assigned something different through unit propagation and eventually you leave the set of preferred literals behind and use the remaining unassigned variables to reach a fully satisfied model.

je kleiner präferenzenmenge, desto wahrscheinlicher komplett im widerspruch mit backbone, alle kollidieren, alle präferenzen müssen durch gelernte literale ausgehebelt werden

bei großem pool an präferenzen kann man nicht funktionierende hinten in eine queue stellen

Also, since the preferred assignments have to be done earlier than others, they occur at the top of the CDCL table. This means that in case of a conflict with other assignments, the preferred decisions will not be reverted if there is any other decision available to be reverted, since the strategy of CDCL is most likely to revert only the youngest decision involved with the conflict. But if this preference actually goes against a backbone literal of the formula, it must eventually be reverted to reach a valid model. This can only happen by a unit propagation, since a decision would just take the preference into account again. <- pb1c hat das problem in jedem satcall

You can actually see this effect with all the other computation schemes in table \ref{tab:satCompAvg} when you look at the $t_{last}$ column. All of these timings are larger than the average computation would take (if you divide $t_{sat}$ by $n_{sat}$) and the last SAT call is also the one where the set of preferences is the smallest. However, $PB1c$ with a sufficiently low number of preferences has this problem in all of it's SAT calls.
%,especially since most of the preferences at that point stand against the formulas backbone.
A potential explanation for this could be that a small set of pref

 ??? TODO überlegen wieso oder rausnehmen

Kleine mengen an präferenzen machen lösen tatsächlich schwieriger, kleiner pool an auswahlmöglichkeiten (sonst gehen in präferenzmenge noch auswahlheuristiken)
...
Table \ref{tab:satCompForgettingBenefits}\footnote{
	These performance ratings differ slightly from table \ref{tab:satCompAvg}. This is because that is a different execution.
} lists a comparison of multiple backbone algorithms once with ordinary, permanent preferences and in a forgetting scheme as described in section \ref{sec:amnesPrefs}. Preferences in general resulted in worse performance for this benchmark. However, if configured to immediately drop those preferences involved in a conflict, the penalty was always reduced to an acceptable level.

\begin{wraptable}[12]{r}{7cm} %[tbp]
\begin{tabular}{l| c c }
...
Table \ref{tab:vonThore1} in the upcoming section shows results for a formula that is more beneficient to preferences. Here we see, that in such a case, forgetting preferences still work as intended. If these results turn out to be reproducible for other formulas as well, forgetting preferences could be a viable strategy to compute the backbone of any formula without prior knowledge about it, since the penalty for difficult formulas would be relatively low but the speedup for easy formulas very high.
