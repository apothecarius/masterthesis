...
PB3 & 20.927 & 20.922 & 4 \\
\end{tabular}
\caption{Averages of 65 testfiles taken from sat competitions. The columns indicate: The full time that the calculation took in seconds; The time that was spent in the sat solver; The number of sat calls (all three values are averages). }
...
Table \ref{tab:satCompAvg} shows an extreme performance drop for the $PB1c(5\%)$ case.
...
tab \ref{tab:satCompForgettingBenefits}\footnote{These performance ratings differ slightly from table \ref{tab:satCompAvg}. This is because that is a different execution.}

schlechte ergebnisse von PB1c (0.05) vgl run2 mit run3 (run3 hat kein default vergessen)

TODO solver mit vergessenden präferenzen ausführen, mindestens PB1c(5\%) sollte nicht ohne vergessende ausgeführt werden, weil das sonst 2 Tage dauert. Muss neu gemacht werden, weil fallback präferenz in ThirdRun anders ist als in SecondRun.