...
Table \ref{tab:vonThore1} lists experimental results with the same set of algorithms that were run on the files from the SAT competition. After having analyzed these performance results, I will try to give a specialized algorithm that is optimized for this particular benchmark in the next section.

We immediately see, that preferences give a much greater benefit than in the previous benchmark with the files from the SAT competition. However there are some more things that stand out:
\begin{itemize}
...
\subsection{Reuse of backbone information}
\begin{wraptable}{r}{4.5cm}
%\begin{table}[tbp]
\begin{tabular}{l|  c c }
Strategy & $t_{complete}$ & $n_{sat}$ \\
\hline
$BB$ & 123.321 & 1337 \\
$BB_{pre}$ & 321.123 & 3117 \\
$IBB$ & 123.321 & 1337 \\
$IBB_{pre}$ & 321.123 & 3117 \\
$PB0$ & 123.321 & 1337 \\
$PB0_{pre}$ & 321.123 & 3117 \\
$PB4$ & 123.321 & 1337 \\
$PB4_{pre}$ & 321.123 & 3117 \\
\end{tabular}
\end{wraptable}
The special structure of this use case should allow us to reuse information between the computations. If we can determine backbone literals of the base formula $F_{conf}$, then we can safely assume, that all of these are part of any restricted formula $F_{conf} \cup \{a\}$. This is because, when we add any clause to a CNF formula, it's set of models can only shrink, no new model can be induced through an additional clause. This implies that an assignment that didn't occur in the models before will also not appear in the new smaller set of models, in turn implying that if a a variable always occured with the same assignment in $F_{conf}$, it will also do so in $F_{conf} \cup \{a\}$.

This means, that a lot of work that otherwise would have to be repeated for all the tested literals can be computed once and just be reused.

TODO tabelle mit und ohne pre run
{BB, IBB, PB0, PB4}x {mit prerun / ohne}
